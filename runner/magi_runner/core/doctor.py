...
def run_doctor(config_path: Path) -> dict[str, Any]:
    ...
    metasploit = metasploit_capability()
    add("metasploit", bool(metasploit.get("available")), f"{metasploit.get('version') or 'unknown'} | {metasploit.get('path') or metasploit.get('message', '')}")
