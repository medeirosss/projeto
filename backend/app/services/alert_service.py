...
def build_alert_record(normalized: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "event": str(normalized.get("event") or normalized.get("event_number") or normalized.get("display_name") or "Inbound Alert"),
...
def create_alert_from_inbound(normalized: Dict[str, Any]) -> Dict[str, Any]:
    alert_id = str(normalized.get("alert_id") or "")
