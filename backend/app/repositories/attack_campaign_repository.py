...
def ensure_attack_campaign_schema() -> None:
    with SessionLocal() as db:
        ...
        CREATE TABLE IF NOT EXISTS attack_campaigns (
          ...
          windows_credential_ids JSONB NOT NULL DEFAULT '[]'::jsonb,
          ...
          snmp_topology_enabled BOOLEAN NOT NULL DEFAULT TRUE,
          dhcp_enabled BOOLEAN NOT NULL DEFAULT FALSE,
          dhcp_server VARCHAR(255),
          dhcp_credential_id INTEGER,
        ...
        CREATE TABLE IF NOT EXISTS attack_campaign_paths (
          ...
          created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
          finished_at TIMESTAMP,
          UNIQUE(execution_id, origin, target, protocol, credential_id)
        );
        ...
        ALTER TABLE attack_campaigns ADD COLUMN IF NOT EXISTS windows_credential_ids JSONB NOT NULL DEFAULT '[]'::jsonb;
        ...
        ALTER TABLE attack_campaigns ADD COLUMN IF NOT EXISTS snmp_topology_enabled BOOLEAN NOT NULL DEFAULT TRUE;
        ALTER TABLE attack_campaigns ADD COLUMN IF NOT EXISTS dhcp_enabled BOOLEAN NOT NULL DEFAULT FALSE;
        ALTER TABLE attack_campaigns ADD COLUMN IF NOT EXISTS dhcp_server VARCHAR(255);
        ALTER TABLE attack_campaigns ADD COLUMN IF NOT EXISTS dhcp_credential_id INTEGER;
    ...
    with SessionLocal() as db:
        row = db.execute(text("""
          INSERT INTO attack_campaigns(
            campaign_uuid,name,description,scope_cidrs,initial_seeds,credential_id,windows_credential_ids,ssh_credential_id,snmp_credential_id,enabled_vectors,create_benign_evidence,snmp_topology_enabled,dhcp_enabled,dhcp_server,dhcp_credential_id,runner_id,
          ...
          VALUES(:uuid,:name,:description,CAST(:scope AS JSONB),CAST(:seeds AS JSONB),:credential_id,CAST(:windows_credential_ids AS JSONB),NULL,:snmp_credential_id,CAST(:enabled_vectors AS JSONB),:create_benign_evidence,:snmp_topology_enabled,:dhcp_enabled,:dhcp_server,:dhcp_credential_id,:runner_id,
        ...
        """), {
            ...
            "credential_id": data.get("credential_id"), "windows_credential_ids": _json(data.get("windows_credential_ids") or []), "ssh_credential_id": None, "snmp_credential_id": data.get("snmp_credential_id"), "enabled_vectors": _json(data.get("enabled_vectors") or ["winrm","smb","snmp_v2c"]), "create_benign_evidence": bool(data.get("create_benign_evidence")), "snmp_topology_enabled": bool(data.get("snmp_topology_enabled",True)), "dhcp_enabled": bool(data.get("dhcp_enabled")), "dhcp_server": data.get("dhcp_server"), "dhcp_credential_id": data.get("dhcp_credential_id"), "runner_id": data.get("runner_id"),
...
def get_campaign(campaign_uuid: str) -> dict[str, Any] | None:
    with SessionLocal() as db:
        ...
        if executions:
            ...
            out["dhcp_leases"] = [dict(x) for x in db.execute(text("SELECT * FROM attack_campaign_dhcp_leases WHERE execution_id=:e ORDER BY ip_address"), {"e":eid}).mappings().all()]
            out["coverage"] = [dict(x) for x in db.execute(text("SELECT * FROM attack_campaign_coverage WHERE execution_id=:e ORDER BY ip_address"), {"e":eid}).mappings().all()]
        return out
