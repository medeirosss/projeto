...
def ensure_validation_schema() -> None:
    ...
    with SessionLocal() as db:
        ...
        db.execute(text("""CREATE TABLE IF NOT EXISTS validation_task_executions (id SERIAL PRIMARY KEY,execution_uuid UUID NOT NULL DEFAULT gen_random_uuid(),validation_task_id INTEGER REFERENCES validation_tasks(id) ON DELETE SET NULL,repository_key VARCHAR(80) NOT NULL,task_key VARCHAR(160) NOT NULL,runner_id VARCHAR(100),runner_job_id INTEGER REFERENCES runner_jobs(id) ON DELETE SET NULL,target VARCHAR(255) NOT NULL,target_id INTEGER REFERENCES targets(id) ON DELETE SET NULL,requested_by VARCHAR(160),status VARCHAR(50) NOT NULL DEFAULT 'queued',impact VARCHAR(30),plan JSONB NOT NULL DEFAULT '{}'::jsonb,evidence JSONB NOT NULL DEFAULT '{}'::jsonb,finding_status VARCHAR(30),finding_message TEXT,remediation TEXT,error TEXT,created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,started_at TIMESTAMP,finished_at TIMESTAMP)"""))
        db.execute(text("ALTER TABLE validation_task_executions ADD COLUMN IF NOT EXISTS target_id INTEGER REFERENCES targets(id) ON DELETE SET NULL"))
        db.execute(text("CREATE INDEX IF NOT EXISTS idx_validation_executions_target ON validation_task_executions(target_id)"))
        db.commit()
...
def create_execution(task:dict, runner_id:str, runner_job_id:int, target:str, requested_by:str, plan:dict):
    with SessionLocal() as db:
        ...
        target_id=resolve_target_id(db,target)
        row=db.execute(text("""INSERT INTO validation_task_executions(validation_task_id,repository_key,task_key,runner_id,runner_job_id,target,target_id,requested_by,status,impact,plan,remediation) VALUES(:task_id,:repo,:key,:runner,:job,:target,:target_id,:requested,'queued',:impact,CAST(:plan AS JSONB),:remediation) RETURNING *"""),{"task_id":task["id"],"repo":task["repository_key"],"key":task["task_key"],"runner":runner_id,"job":runner_job_id,"target":target,"target_id":target_id,"requested":requested_by,"impact":task.get("impact"),"plan":_json(plan),"remediation":task.get("remediation")}).mappings().first(); db.commit(); return dict(row)
