...
def create_atomic_lab_execution_service(
    atomic_test_id: int,
    runner_id: str,
    ...
    with SessionLocal() as db:
        ...
        if str(test["risk_level"]).lower() != "low":
            blockers.append(f"Risco precisa ser low. Atual: {test['risk_level']}")

        if str(test["executor_name"]).lower() != "powershell":
            blockers.append(f"Executor precisa ser powershell. Atual: {test['executor_name']}")

        if bool(test["requires_reboot"]):
            blockers.append("Teste exige reboot.")

        if bool(test["executor_elevation_required"]):
            blockers.append("Teste exige privilégio elevado.")

        if bool(test["has_dependencies"]) or int(test["dependency_count"] or 0) > 0:
            blockers.append("Teste possui dependências. Use primeiro testes sem dependências.")
        ...
        payload = {
            "mode": "execute_lab",
            "risk_level": str(test["risk_level"]).lower(),
            "atomic_name": test["display_name"],
